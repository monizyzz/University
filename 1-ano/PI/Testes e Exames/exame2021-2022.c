...
int pesquisa(int a[], int N, int x) {
    ...
    while (inicio <= fim) {
        ...
        if (a[meio] == x) {
            ...
            inicio = meio + 1; // Procurar na metade direita do array
        } else {
            fim = meio - 1; // Procurar na metade esquerda do array
        }
    }
...
/* Questão 2 */
void roda(LInt *l) {
    // Verificar se a lista está vazia ou tem apenas um elemento
    if (*l == NULL || (*l)->prox == NULL) {
        return;
    }

    LInt last = *l;
    LInt secondLast = NULL;

    // Encontrar o último e o penúltimo elementos da lista
    while (last->prox != NULL) {
        secondLast = last;
        last = last->prox;
    }

    // Movendo o último elemento para a cabeça da lista
    secondLast->prox = NULL;
    last->prox = *l;
    *l = last;
}
